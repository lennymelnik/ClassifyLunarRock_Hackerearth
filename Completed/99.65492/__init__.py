...
test_set = test_datagen.flow_from_directory('output/val',
                                           ...
                                           class_mode='binary',
                                           shuffle=False)
#Test Set with no class data
test_set1 = test_datagen.flow_from_directory('DataSet/Test Images',
                                            target_size=(720,480),
                                            batch_size=32,
                                            shuffle=False)#

#Comment the two following lines if you want to use a pretrained network
...
##Uncomment the bottom when you want to load from the pretrained model
...
for i in range(length_test):
    ...
    prediction = model.predict(img, batch_size=None, steps=1)  # get class prob
    if (prediction[:, :] > 0.5):
